...
from stravaGo.settings import MEDIA_ROOT, MEDIA_URL
import json
...
from reservationApp.models import Booking, Category, Location, Run, Schedule
...
def home(request):
    ...
    context['buses'] = Run.objects.count()
...
def save_bus(request):
    ...
    if request.method == 'POST':
        if (request.POST['id']).isnumeric():
            bus = Run.objects.get(pk=request.POST['id'])
        else:
            bus = None
        if bus is None:
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Joki has been saved successfully.')
            resp['status'] = 'success'
        else:
            for fields in form:
                for error in fields.errors:
...
def delete_bus(request):
    ...
    if request.method == 'POST':
        try:
            bus = Run.objects.get(id = request.POST['id'])
            bus.delete()
            messages.success(request, 'Joki has been deleted successfully')
            resp['status'] = 'success'
        except Exception as err:
...
def manage_schedule(request, pk=None):
    ...
    buses = Run.objects.filter(status = 1).all()
